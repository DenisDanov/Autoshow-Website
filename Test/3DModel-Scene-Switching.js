...
async function initThirdPersonScript() {
    ...
    if (carParam.includes('.glb')) {
        ...
        const worker = new Worker('offscreen_worker.mjs');
        worker.postMessage({ msg: 'offscreen', canvas: offscreenCanvas }, [offscreenCanvas]);

        let loadingProgress = 0;

        worker.addEventListener('message', function (ev) {
            if (ev.data.msg === 'loadModel') {
                const model = ev.data.model;
                onLoad(model);
            } else if (ev.data.msg === 'loadTexture') {
                const texture = ev.data.texture;
                applyTextureToModel(texture);
            } else if (ev.data.msg === 'loadingProgress') {
                loadingProgress = ev.data.progress;
                // Update your loading UI with the loadingProgress value
                updateLoadingUI();
            }
        });
